function [outFilepath, outFilename, EEGsync] = filesync_macromicro ...
    (micro_ns5_filepath, micro_ns5_filename, macro_filepath, macro_filename, outFilepath, delay)
% [outFilepath, outFilename, EEGsync] = FILESYNC_MACROMICRO ...
% (micro_ns5_filepath, micro_ns5_filename, macro_filepath, ...
% macro_filename, out_macro_filepath, delay)
...
%
%  If the delay is known, it can be specified as the 6th argument. Use this
%  when there is no triggers on one on the file. 
%
% Optionnal INPUTS:
...
%   - delay                     : delay between Macro and micro files (if known)
% 
% OUTPUTS:
%   - outFilepath               : output synchronized macro filepath 
...
%% Create the new synchronize EEG structure of the Macro file
