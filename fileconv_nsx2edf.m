function [outFilepath, outFilenames, EEGs] = fileconv_nsx2edf ...
    (inFilepath, inFilename, chunkDuration, downsamplingFactor, outFilepath)
% [outFilepath, outFilenames, EEGs] = FILECONV_NSX2EDF 
...
if nargin<2
    ...
    inFilepath  = NS.MetaTags.FilePath;
    inFilename  = NS.MetaTags.Filename;
else
    NS          = openNSx(fullfile(inFilepath,inFilename));
    if ~isstruct(NS); disp(['Could not open NSx file : ',fullfile(inFilepath,inFilename)]); return; end;
end
...
if chunkDuration<0
    ...
    pop_writeeeg (EEGs,fullfile(outFilepath,outFilenames),'TYPE','EDF');
else
    for iChunk=1:nChunks
        ...
        NSpart_i  	= openNSx (fullfile(inFilepath,inFilename),...
        ...
        pop_writeeeg (EEGs{iChunk},fullfile(outFilepath,outFilenames{iChunk}),'TYPE','EDF');
    end
end
